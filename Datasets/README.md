...
The labeling rule depends only on the target service and never on the signal, so the three signals of one service share a ground truth wherever their window grids agree. The two testbeds also ran one synchronised fault schedule, with fault start times matching to the second, so some otel-demo and sockshop services share a ground truth as well. The 20 series therefore rest on 9 distinct (testbed, service) fault schedules and carry 15 distinct label vectors once the differing window grids are taken into account. Series 203, 209, 212 and 217 share one identical vector of 50 anomalous windows, and 205, 211 and 218 share another of 51. The closest near-duplicates are 201 and 216 at Jaccard 0.981, 210 and 213 at 0.967, 202 and 208 at 0.879, and 208 and 214 at 0.836. `OTel_series_summary.csv` carries `label_vector_id`, `max_label_jaccard` and `max_label_jaccard_with` so the grouping is explicit. Anything that averages a measure across all 20 series is weighting correlated ground truth. Taking one series per `label_vector_id`, or one per service, gives an uncorrelated subset.
